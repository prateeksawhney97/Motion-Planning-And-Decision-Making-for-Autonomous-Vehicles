...
std::vector<State> MotionPlanner::generate_offset_goals(
  ...
  auto yaw_plus_90 = M_PI_2+goal_state.rotation.yaw; // <- Fixed This
  //auto yaw = ;  // <- Fix This
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    goal_offset.location.x = goal_offset.location.x + std::cos(yaw_plus_90); // <- Fixed This 
    goal_offset.location.y = goal_offset.location.y + std::sin(yaw_plus_90); // <- Fixed This
