...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  //auto look_ahead_distance = 1.0;  // <- Fixed This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      //goal.location.x += 1.0;  // <- Fixed This
      //goal.location.y += 1.0;  // <- Fixed This
      ...
      goal.velocity.x = 0;  // <- Fixed This
      goal.velocity.y = 0;  // <- Fixed This
      goal.velocity.z = 0;  // <- Fixed This
      ...
      goal.velocity.x = std::cos(goal.rotation.yaw)*_speed_limit;  // <- Fixed This
      goal.velocity.y = std::sin(goal.rotation.yaw)*_speed_limit;  // <- Fixed This
      goal.velocity.z = 0;
    }
    ...
    // if (utils::magnitude(ego_state.velocity) <= _stop_threshold_speed) {  // -> Fixdc this
      ...
      _active_maneuver = STOPPED;  // <- Fixed This
    ...
    // the stop line. goal = ...;
       goal = _goal;  // Keep previous goal. Stay where you are. // <- Fixed This
    ...
    if (stopped_secs >= _req_stop_time && tl_state.compare("Red") != 0) {
      ...
      _active_maneuver = FOLLOW_LANE;  // <- Fixed This
